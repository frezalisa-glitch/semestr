...
//интерфейс
/*Он описывает правила, которым должны следовать все ручки, но не содержит самой логики.*/
class InterfacePen {
public:
...
InterfacePen::~InterfacePen() {} // реализация деструктора (виртуальный деструктор должен иметь тело).
...
public:
    ...
    virtual void Show() const;           //это функция для вывода информации на экран. Она может быть переопределена в наследниках (полиморфизм) и гарантированно не меняет данные ручки
...
private:
    double tipSize;     // поле: размер шарика (в мм).
    static int numBall; // счетчик именно для шариковых ручек.
